...
it('fails when queue is not faked', function () {
    expect(fn () => PlainQueueableExecutable::assert()->queued()->withArgs(fn ($input) => $input === 123))
...
it('passes when executable was queued with given input', function () {
    ...
    PlainQueueableExecutable::assert()->queued()->withArgs(fn ($input) => $input === 'input');
});
...
it('fails when executable was not queued with given input', function () {
    ...
    $assertion = PlainQueueableExecutable::assert()->queued()->withArgs(fn ($input) => $input === 'other-input');
